...
ControlInformation <- function(Product,RUN_NR)
{
  ...
  for(j in 1:nrow(Investigation_Periods))
  {
    ...
    NR_POLS_IF <- nrow(PolicyData_j_Active)
    PREM_IF    <- sum(PolicyData_j_Active$PREM)
    SUM_ASS_IF <- sum(PolicyData_j_Active$SUM_ASS)
    
    PolicyData_j_NB <- PolicyData_j[which(PolicyData_j$ENTRY >= extractstart),]

    ControlOutput <- data.frame(Build_up = c("Opening","NB","Deaths","Lapses","Disability","Closing","Check"),
               NR_POLS  = c(10000    ,2000,500     ,1000    ,75          ,100000, 0),
               PREM = 1,
               SUM_ASS = 1)

    #    PolicyData_j <- PolicyData[PolicyData$STATUS %in% c("A") & PolicyData$ENTRY <= extractend,]
...
Run_User_defined_output(RUN_NR = 200) # Control output
